...
$convert_options = "-v -HumanReadable $var_pr";
...
#  }
  ...
  print ("Converting set [$convert_path -f $dir/$filebase -np $nproc $convert_options]\n");
  ...
  if ( -e "model_params.in" ) {
    system("cd $dir; cp ../model_params.in .");
  } 

  # copy traction.in file
  if ( -e "traction.in" ) {
      system("cp traction.in $dir");
   }
